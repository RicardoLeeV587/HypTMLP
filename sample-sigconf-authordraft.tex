...
\begin{equation}
    \log_{\mathbf{0}}(\bm{x}) = \frac{2}{\sqrt{c}} \text{arctanh}(\sqrt{c}\|\bm{x}\|) \frac{\bm{x}}{\|\bm{x}\|}.
\end{equation}
...
Table \ref{tab:efficiency} presents the FLOPs and parameter counts for a representative dataset (e.g., \textit{MagicTelescope}). HDE-Net requires significantly fewer FLOPs (approx. $\mathbf{1/10}\times$) compared to FT-Transformer while maintaining a compact parameter size. This confirms that our ``heavy-representation, light-backbone'' design effectively offloads the reasoning burden from the expensive backbone to the efficient embedding layer.
\begin{table}[h]
    \centering
    \caption{\textbf{Efficiency Comparison.} FLOPs and Parameters are estimated on the \textit{MagicTelescope} dataset ($N_{feat}=10$, Binary Classification). HDE-Net achieves SOTA performance with minimal computational cost.}
    \label{tab:efficiency}
    \resizebox{0.9\linewidth}{!}{
    \begin{tabular}{l|cc|c}
    \toprule
    \textbf{Model} & \textbf{Params (M)} & \textbf{FLOPs (M)} & \textbf{Avg Rank} \\
    \midrule
    FT-Transformer & 0.23 & 0.85 & 11.63 \\
    RealMLP & 0.84 & 1.68 & 8.90 \\
    ModernNCA & 0.02 & $N_{train} \times 0.001^*$ & 8.60 \\
    \midrule
    \textbf{HDE-Net (Ours)} & \textbf{0.39} & \textbf{0.39} & \textbf{4.17} \\
    \bottomrule
    \end{tabular}
    }
%    \begin{tablenotes}
%        \small
%        \item[*] ModernNCA's inference cost grows linearly with the training set size, making it prohibitive for large-scale data.
%    \end{tablenotes}
\end{table}

\noindent\textit{Implementation Note:} While HDE-Net exhibits superior theoretical efficiency, its current wall-clock inference speed is constrained by the lack of kernel optimization for Riemannian operations (e.g., Möbius Operations) in standard libraries like \texttt{geoopt}. We anticipate that future engineering optimizations, such as operator fusion, will bridge the gap between theoretical FLOPs and real-world latency.
