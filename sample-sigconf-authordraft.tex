...
In this work, we propose to bridge this gap not by patching DNN architectures, but by establishing a geometric isomorphism with the inference logic of decision trees. We introduce \textbf{Hyperbolic Decision Embedding Network (HDE-Net)} that achieves true value-level unification for heterogeneous features using Poincaré ball embeddings \cite{nickel2017poincare, becigneul2018riemannian, peng2021hyperbolic}.
...
\begin{itemize}
    \item \textbf{Tree-Isomorphic Manifold Representation}: We propose a new paradigm that semantically discretizes numerical features into LDNs, resolving the representation granularity mismatch in existing tabular DNN frameworks.
    ...
    \item \textbf{Adaptive Semantic Capacity}: We introduce the entropy-aware capacity allocation algorithm to balance numerical LDNs' expressiveness and efficiency, preventing over- and under-parameterization.
...
The rest of this paper is organized as follows. Section \ref{sec:related_work} reviews related work of current tabular DNNs and Hyperbolic DNNs. Section 3 provides preliminaries on hyperbolic geometry and necessary operators. Section 4 details the proposed HDE-Net. Section 5 presents experimental results and analysis, followed by conclusions in Section 6.
...
\textbf{(1) Ad-hoc Fusion} applies distinct processing pipelines to different feature types. For instance, RealMLP \cite{holzmuller2025realmlp} employs Periodic-Bias-Linear-Dense (PBLD) embeddings for numerical features and value-level embeddings for categorical ones, fused via residual blocks. TabTransformer \cite{huang2020tabtransformer} processes categorical features via Transformers while concatenating raw numerical features directly. 
\textbf{(2) Pseudo-Alignment} forces dimensional alignment via tokenizers. The representative FT-Transformer \cite{gorishniy2021revisiting} projects numerical features via linear layers ($x \cdot W + b$) and looks up categorical embeddings, processing them uniformly through a deep Transformer backbone. Similar mechanisms are adopted by AutoInt \cite{song2019autoint}, ExcelFormer \cite{chen2023excelformer}, and T2G-Former \cite{yan2023t2g}.
\textbf{(3) Retrieval-based Methods}, such as ModernNCA \cite{ye2024revisiting} and TabR \cite{gorishniy2023tabr}, enhance prediction by retrieving similar samples from the training set. While achieving high performance, they incur high inference latency and storage costs, and are sensitive to out-of-distribution shifts.
\textbf{(4) Foundation Models} like TabPFN \cite{hollmann2022tabpfn} leverage Prior-Data Fitted Networks (PFNs) pre-trained on synthetic datasets. Despite their zero-shot capabilities, they face scalability bottlenecks regarding context length ($N$) and feature dimensions.

\subsection{Empirical Analysis of Architectural Limitations}
To rigorously analyze the intrinsic architectural efficacy, we focus on the first two paradigms (Ad-hoc Fusion and Pseudo-Alignment) and compare them against GBDTs under comparable computational budgets. We exclude Retrieval and Foundation models from this specific analysis due to their disparate resource requirements. We conduct a performance breakdown on the TALENT benchmark (see Table \ref{tab:motivation}) across datasets grouped by feature composition: \textit{Overall} (All datasets), \textit{Numerical-Only} (Datasets only contains numerical features), \textit{Numerical-Heavy} (Numerical dominant datasets), and \textit{Categorical-Heavy} (Categorical dominant datasets).
...
The results reveal critical insights: 
\begin{itemize}
...
Let $\mathcal{M} = \mathbb{B}_c^d$ be the $d$-dimensional Poincaré ball with curvature $c$. For all features' LDNs (numerical and categorical), we initialize their corresponding embeddings (HDEs) $\bm{Z}$ directly on the same manifold $\mathcal{M}$.
%\begin{equation}
...
%To enable efficient inference and compatibility with standard neural layers, we decouple the geometric representation from the aggregation logic. Instead of performing expensive Möbius operations on the manifold, we project the retrieved HDEs $\bm{Z}^r$ to the Euclidean tangent space at the origin $\mathcal{T}_{\mathbf{0}}\mathbb{B}_c^d$ using the Logarithmic map:
%\begin{equation}
%    \bm{V}^r = \log_{\mathbf{0}}(\bm{Z}^r) \quad \bm{Z}^r \subset \bm{Z}
%\end{equation}
...
%\begin{equation}
%    \mathbf{e}^{num}_j = \frac{1}{k_j}\sum_{n=1}^{k_j} w_{j,n} \mathbf{v}_{j,n},  \quad \text{where } [w_{j,1}, \dots, w_{j, k_j}] =  \boldsymbol{W}_j
%\end{equation}
...
To further investigate the adaptability of HDE-Net to different feature compositions, we analyze the average ranking across three specific scenarios: \textit{Num-only} (Datasets only contains numerical features), \textit{Num-Heavy} (Numerical dominant datasets), and \textit{Cat-Heavy} (Categorical dominant datasets). Since the benchmark contains only one categorical-only dataset (\texttt{ozone\_level}), it is merged into the categorical-dominant group. We select representative models from each paradigm for comparison: XGBoost (Tree), RealMLP (Ad-hoc Fusion), FT-Transformer (Pseudo-Alignment), and ModernNCA (Retrieval). The results are illustrated in Figure \ref{fig:scenarios}.
...
\begin{itemize}
    ...
    \item \textbf{Hyperbolic Geometry as the Crown Jewel.} While Euclidean routing is effective, introducing the hyperbolic manifold elevates the performance from Rank 7.10 to \textbf{4.93}. This substantial leap confirms that the implicit hierarchical constraints offered by the Poincaré ball provide a unique inductive bias that Euclidean spaces cannot replicate, essential for mastering complex decision boundaries.
    
    \item \textbf{High Capacity with Low Dimensionality.} Even in its weakest form, \textit{HDE-noRouter} (Rank 13.45) achieves parity with FT-Transformer (13.93). Crucially, HDE operates with a minimal 12-dimensional embedding and a simple MLP, whereas FT-Transformer relies on 100+ dimensions and a heavy Transformer backbone. This underscores the exceptional data efficiency of hyperbolic representations.
\end{itemize}
...
To evaluate the computational efficiency of HDE-Net, we compare its theoretical complexity and resource usage against leading baselines. Since wall-clock time is heavily dependent on hardware-specific optimizations (e.g., CUDA kernel fusion for Transformers), we focus on hardware-agnostic metrics: \textit{FLOPs} (Floating Point Operations) per sample and \textit{Model Parameters}.

\noindent\textbf{Theoretical Complexity.}
Structure-based models like FTTransformer rely on self-attention mechanisms, resulting in a computational complexity of $\mathcal{O}(L \cdot N_{feat}^2)$, where $L$ is the number of layers and $N_{feat}$ is the number of features. Retrieval-based methods like TabR and ModernNCA require searching nearest neighbors from the training set size $N_{train}$ at inference time, leading to a complexity of $\mathcal{O}(N_{train} \cdot d)$.
...
\end{table}%
 ...
 As detailed in Table \ref{tab:efficiency}, HDE-Net achieves a remarkable balance between efficiency and performance:
...
\end{figure}
\begin{itemize}
	\item \textbf{Branching Pattern Emergence}: Starting from a random initialization (Epoch 0), the LDNs progressively organize into distinct clusters with fractal-like branching patterns, mirroring the topology of decision trees. 
